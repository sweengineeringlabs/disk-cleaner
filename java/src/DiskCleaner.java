 ...
 * Thin CLI entry point — dispatches to feature saf factories.
 */
public class DiskCleaner {
    ...
    public static void main(String[] args) {
        ...
        if ("help".equals(cli.command)) {
            showHelp();
            return;
        }

        var configPath = cli.config != null ? cli.config : "profiles.toml";
        var config = ConfigFactory.loadConfig(configPath);
        if (config == null) {
            System.err.println("Error: could not load config: " + configPath);
            System.exit(1);
        }

        var searchPath = cli.path != null ? cli.path : config.getValue("settings.default_path");
        if (searchPath.isEmpty()) searchPath = System.getProperty("user.dir");

        var lang = cli.lang;
        if (lang.length == 0) lang = config.getArray("settings.default_profiles");

        var command = cli.command != null ? cli.command : "clean";

        if ("list-profiles".equals(command)) {
            listProfiles(config);
            return;
        }

        var profileKeys = resolveProfiles(lang, config);
        if (profileKeys == null) System.exit(1);

        // Composition root — create scanner, dispatch to features
        var ctx = ConfigFactory.createScanner(searchPath, cli.exclude, cli.include, cli.all, cli.jsonOutput);
        var configDir = new java.io.File(configPath).getParent();
        if (configDir == null) configDir = ".";

        switch (command) {
            case "clean":
                CleanFactory.run(ctx, profileKeys, config, cli.dryRun, cli.parallel);
                if (!ctx.isCancelled() && !cli.dryRun) {
                    MonitorFactory.saveHistory(configDir, "clean",
                            String.join(", ", profileKeys), ctx.totalCleaned,
                            ctx.totalSizeBytes, ConfigFactory.formatSize(ctx.totalSizeBytes), searchPath);
                }
                break;
            case "search":
                SearchFactory.run(ctx, profileKeys, config, cli.text);
                break;
            case "analyze":
                if (cli.diskUsage) {
                    AnalyzeFactory.runDiskUsage(ctx, cli.depth);
                } else {
                    AnalyzeFactory.run(ctx, profileKeys, config, cli.benchmark);
                }
                break;
            case "monitor":
                MonitorFactory.run(ctx, configDir, cli.history);
                break;
            case "compact-wsl":
                CompactWslFactory.run(ctx, cli.dryRun);
                break;
            default:
    ...
    static void listProfiles(main.features.config.api.ConfigProvider config) {
        System.out.println("Available profiles:");
        System.out.println();
        var keys = config.profileKeys();
        for (int i = 0; i < keys.length; i++) {
            var profile = ConfigFactory.loadProfile(keys[i], config);
    ...
    static String[] resolveProfiles(String[] lang, main.features.config.api.ConfigProvider config) {
        var resolved = new String[0];
        for (int i = 0; i < lang.length; i++) {
            if ("all".equals(lang[i])) return config.profileKeys();
            var name = config.getValue("profiles." + lang[i] + ".name");
            if (name.isEmpty()) {
                System.err.println("Unknown profile: " + lang[i]);
                System.err.println("Use list-profiles to see available profiles.");
                return null;
            }
            resolved = appendStr(resolved, lang[i]);
        }
    ...
    static CliArgs parseArgs(String[] args) {
        ...
        while (i < args.length) {
            var arg = args[i];
            switch (arg) {
                case "--lang": case "-l": i++; if (i < args.length) langList = appendStr(langList, args[i]); break;
                case "--path": case "-p": i++; if (i < args.length) cli.path = args[i]; break;
                case "--config": case "-c": i++; if (i < args.length) cli.config = args[i]; break;
                case "--exclude": case "-e": i++; if (i < args.length) excludeList = appendStr(excludeList, args[i]); break;
                case "--include": case "-i": i++; if (i < args.length) includeList = appendStr(includeList, args[i]); break;
                case "--all": case "-a": cli.all = true; break;
                case "--json": case "-j": cli.jsonOutput = true; break;
                case "--dry-run": case "-d": cli.dryRun = true; break;
                case "--parallel": case "-P": cli.parallel = true; break;
                case "--text": case "-t": i++; if (i < args.length) cli.text = args[i]; break;
                case "--disk-usage": cli.diskUsage = true; break;
                case "--depth": i++; if (i < args.length) cli.depth = Integer.parseInt(args[i]); break;
                case "--benchmark": cli.benchmark = true; break;
                case "--history": cli.history = true; break;
                case "--help": case "-h": cli.command = "help"; break;
                default: if (!arg.startsWith("-") && cli.command == null) cli.command = arg; break;
            }
            i++;
        }
